...
import com.android.backchina.dao.ChannelDao;
...
public class ChannelManage {
	...
	public List<ChannelItem> getUserChannel() {
		Object cacheList = channelDao.listCache(DataBaseHelper.CHANNEL_SELECTED + "= ?",new String[] { "1" });
		if (cacheList != null && !((List) cacheList).isEmpty()) {
			...
			for (int i = 0; i < count; i++) {
				...
				navigate.setId(Integer.valueOf(maplist.get(i).get(DataBaseHelper.CHANNEL_ID)));
				navigate.setName(maplist.get(i).get(DataBaseHelper.CHANNEL_NAME));
				navigate.setOrderId(Integer.valueOf(maplist.get(i).get(DataBaseHelper.CHANNEL_ORDERID)));
				navigate.setSelected(Integer.valueOf(maplist.get(i).get(DataBaseHelper.CHANNEL_SELECTED)));
				list.add(navigate);
			}
			return list;
		}
	...
	public List<ChannelItem> getOtherChannel() {
		Object cacheList = channelDao.listCache(DataBaseHelper.CHANNEL_SELECTED + "= ?" ,new String[] { "0" });
		List<ChannelItem> list = new ArrayList<ChannelItem>();
		if (cacheList != null && !((List) cacheList).isEmpty()){
			List<Map<String, String>> maplist = (List) cacheList;
			int count = maplist.size();
